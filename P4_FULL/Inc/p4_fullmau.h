...
uint16_t manght[40][80];
unsigned char mangxuatled[20][5][80];
		 ...
		 for(pwm=0,pixel=0x01;pwm<5;pixel<<=1,pwm++){
			 for(cot=0;cot<80;cot++){
					   mangxuatled[hang][pwm][cot]=0;
						 ...
						 if((manght[hang+20][cot] & (pixel<<11))) {mangxuatled[hang][pwm][cot] |= 0x02;}
						 if((manght[hang][cot] & (pixel<<5))) {mangxuatled[hang][pwm][cot] |= 0x08;}
						 if((manght[hang+20][cot] & (pixel<<5))) {mangxuatled[hang][pwm][cot] |= 0x10;}
						 if((manght[hang][cot] & pixel)) {mangxuatled[hang][pwm][cot] |= 0x20;}
						 if((manght[hang+20][cot] & pixel)) {mangxuatled[hang][pwm][cot] |= 0x40;}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	    if(htim->Instance ==TIM1)
			{
				 for(int ___i=0;___i<80;___i++)
			   {
					  GPIOB->ODR = mangxuatled[hang][tgian%5][___i];			 
					  GPIOB->ODR |= 0x0080;
				 }
				   GPIOA -> ODR |= (1<<5);
